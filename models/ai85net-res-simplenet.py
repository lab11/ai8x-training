...
class AI85ResidualSimpleNetFC(nn.Module):
    """
    Residual SimpleNet v1 Model
    """
    def __init__(
            ...
            num_classes=2,
            num_channels=3,
            dimensions=(224, 224),  # pylint: disable=unused-argument
            bias=False,
            **kwargs
    ):
        super().__init__()
        ...
        self.conv11 = ai8x.FusedMaxPoolConv2dReLU(96, 512, 1, pool_size=2, pool_stride=2,
                                                  padding=0, bias=bias, **kwargs)
        self.conv12 = ai8x.FusedConv2dReLU(512, 128, 1, stride=1, padding=0, bias=bias, **kwargs)
        self.conv13 = ai8x.FusedMaxPoolConv2dReLU(128, 128, 3, pool_size=2, pool_stride=14,
                                                  stride=1, padding=1, bias=bias, **kwargs)
        self.fc     = ai8x.Linear(128, num_classes, wide=True)

    def forward(self, x):  # pylint: disable=arguments-differ
        """Forward prop"""
        x = self.conv1(x)          # 16x224x224
        x_res = self.conv2(x)      # 20x224x224
        x = self.conv3(x_res)      # 20x224x224
        x = self.resid1(x, x_res)  # 20x224x224
        x = self.conv4(x)          # 20x224x224
        x_res = self.conv5(x)      # 20x112x112
        x = self.conv6(x_res)      # 20x112x112
        x = self.resid2(x, x_res)  # 20x112x112
        x = self.conv7(x)          # 44x112x112
        x_res = self.conv8(x)      # 48x56x56
        x = self.conv9(x_res)      # 48x56x56
        x = self.resid3(x, x_res)  # 48x56x56
        x = self.conv10(x)         # 96x28x28
        x = self.conv11(x)         # 512x14x14
        x = self.conv12(x)         # 128x14x14
        x = self.conv13(x)         # 128x1x1
        x = x.view(x.size(0), -1)
        x = self.fc(x)         # num_classesx1x1
        return x
...
def ai85ressimplenetfc(pretrained=False, **kwargs):
    """
    Constructs a Residual SimpleNet v1 model.
    """
    assert not pretrained
    return AI85ResidualSimpleNetFC(**kwargs)
